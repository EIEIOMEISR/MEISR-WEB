        ...
        for i, q in enumerate(Question.objects.all()):
            ...
            self.fields['custom_%s' % i] = forms.ChoiceField(
                ...
                widget=forms.RadioSelect(
                    attrs={'question': q, 'header': header, 'class': 'inline', 'qid': q.id}),
...
class SignupForm(forms.Form):
    birth_date = forms.DateField(
        required=True, label="Your child's date of birth (mm/dd/yyyy)", widget=DateInput())
...
class ContactForm(forms.Form):
    ...
    message = forms.CharField(widget=forms.Textarea(
        attrs={'placeholder': 'Enter your message'}), required=True)
